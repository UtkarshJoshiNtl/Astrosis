...
const PLOTS = [
  {
    ...
    caption:
      "24-hour LEO propagation (1,000 satellites, 400 km altitude, circular orbits). RK4 at dt=10s (86,400 steps). Relative energy drift: 9.1 × 10⁻⁹ — better than theoretical O(dt⁴) bound of 1 × 10⁻⁷.",
  ...
  },
  {
    ...
    caption:
      "ISS (NORAD 25544) propagated for 24 hours. Position error growth is expected due to TLE uncertainty (0.1–1 km inherent). Astrosis validation: perturbation model behaves consistently with SGP4.",
  ...
  },
  {
    ...
    caption:
      "Circular 700 km LEO, 60° inclination, 7 days. Analytical dΩ/dt = -3.14°/day. RK4: -3.11°/day. Error: +0.96% — J₂ gravity model correctly integrated.",
    status: "pass" as const,
  ...
  },
  {
    ...
    caption:
      "Richardson extrapolation: error ratio ≈ 16 per dt halving confirms 4th-order accuracy (2⁴ = 16). Position error at dt=1.25s: 3.2 × 10⁻⁸ km.",
    status: "pass" as const,
  ...
  },
  {
    ...
    caption:
      "Low-mass (2 kg/m²) vs. high-mass (100 kg/m²) satellite. SRP acceleration ratio: 50× (exact match). 24h displacement: 1.9 km (low) vs. 38 m (high).",
    status: "pass" as const,
...
const GITHUB_RAW =
  "https://raw.githubusercontent.com/UtkarshJoshiNtl/Astrosis/alpha/validation/plots";
        ...
        <div className="grid grid-cols-2 gap-4">
          {PLOTS.map((plot) => (
            <div key={plot.id} className="surface hairline p-3 relative">
              <div
                className="absolute top-2 right-2 flex items-center gap-1 px-1.5 py-0.5 hairline text-[10px] font-mono"
                style={{
                  background:
                    plot.status === "pass"
                      ? "rgba(74, 158, 107, 0.15)"
                      : "rgba(200, 136, 42, 0.15)",
              ...
              <p
                ...
                style={{ color: plot.status === "pass" ? "var(--ok)" : "var(--warn)" }}
              >
                {plot.statusText}
              </p>
            </div>
