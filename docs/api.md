...
All public symbols are accessible via `import engine` (lazy-loaded).
...
## Propagation (`engine.core`)

### `engine.rk4_step(state, dt, mjd0=0, area=0, mass=1, cd=2.2, cr=1.5) -> tuple`
...
4. Brent refinement of TCA (Brent minimisation).
...
### `engine.detect_conjunctions(sat_states, debris_states, lookahead=86400, step_s=60, mjd0=0) -> list`

Auto-backend wrapper (CUDA → C++ → Python). Returns list of
`ConjunctionWarning` instances.

### `engine.ConjunctionWarning`
...
### `engine.monte_carlo_pc(sat_samples, deb_samples, dt, steps, threshold_km, mjd0=0) -> float`

Monte Carlo collision probability. Propagates N sample pairs and counts
passes within `threshold_km`. CUDA → Python fallback.

## Ephemeris (`engine.core.ephemeris`)

### `engine.sun_position_eci(mjd) -> tuple`

Sun ECI position in km at given Modified Julian Date. Low-precision VSOP87
(accuracy ~0.01°).

### `engine.moon_position_eci(mjd) -> tuple`

Moon ECI position in km at given Modified Julian Date. Low-precision ELP-2000
(accuracy ~0.1°).

## Coordinate Frames (`engine.geo.frames`)

### `engine.gmst_from_datetime(dt) -> float`

Greenwich Mean Sidereal Time in radians for a given UTC datetime.

### `engine.eci_to_ecef(r_eci, dt) -> ndarray`

ECI → ECEF rotation at given datetime (includes equation of equinoxes).

### `engine.ecef_to_geodetic(r_ecef) -> (lat, lon, alt)`

ECEF cartesian → WGS-84 geodetic (Bowring iterative method).

### `engine.geodetic_to_ecef(lat, lon, alt) -> ndarray`

WGS-84 geodetic → ECEF cartesian.

### `engine.topocentric_aer(r_ecef, lat_rad, lon_rad, alt_km) -> (az, el, range)`

ECEF → topocentric azimuth/elevation/range for a ground station.

## Pass Prediction (`engine.geo.analysis`)

### `engine.report_passes(norad_id, lat, lon, alt, start_dt, hours, dt_step=60, sat_area=10, sat_mass=1000, sat_cd=2.2, min_elevation_deg=10, ingestor=None) -> dict`
...
### `engine.geo.cities.CITIES`

Dict of ~85 city names → `(lat, lon)`.

### `engine.geo.cities.resolve_location(city_or_lat, lon=None) -> (lat, lon, name)`

Resolve a city name or explicit coordinates.

## Visibility (`engine.geo.visibility`)

### `engine.check_eclipse(r_sat, r_sun) -> str`

Returns `"SUNLIGHT"`, `"UMBRA"`, or `"PENUMBRA"` for a satellite position.

### `engine.is_optically_visible(el_rad, r_sat_eci, dt, min_elevation_deg=10) -> bool`
