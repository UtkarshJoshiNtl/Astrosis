...
struct DeviceMem {
    void* ptr;
    explicit DeviceMem(size_t bytes) {
        cudaError_t e = cudaMalloc(&ptr, bytes);
        if (e != cudaSuccess)
            throw std::runtime_error(std::string("cudaMalloc(") + std::to_string(bytes) + "): " + cudaGetErrorString(e));
    }
    ~DeviceMem() { cudaFree(ptr); }
    DeviceMem(const DeviceMem&) = delete;
    DeviceMem& operator=(const DeviceMem&) = delete;
};

struct HostPinnedMem {
    void* ptr;
    explicit HostPinnedMem(size_t bytes) {
        cudaError_t e = cudaHostAlloc(&ptr, bytes, cudaHostAllocDefault);
        if (e != cudaSuccess)
            throw std::runtime_error(std::string("cudaHostAlloc(") + std::to_string(bytes) + "): " + cudaGetErrorString(e));
    }
    ~HostPinnedMem() { cudaFreeHost(ptr); }
    HostPinnedMem(const HostPinnedMem&) = delete;
    HostPinnedMem& operator=(const HostPinnedMem&) = delete;
};
